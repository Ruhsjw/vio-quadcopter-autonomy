...
class OccupancyMap:
    ...
    @staticmethod
    def _aabb_distance(min_a, max_a, min_b, max_b):
        """
        Euclidean distance between two axis-aligned boxes. This replaces
        scipy.spatial.Rectangle, which is unavailable in some SciPy versions.
        """
    ...
    def _init_map_from_world(self):
        ...
        for block in self.world.world.get('blocks', []):
            ...
            block_min = np.array(extent[0::2], dtype=float)
            block_max = np.array(extent[1::2], dtype=float)
                ...
                for j in range(outer_min_index[1], outer_max_index[1]+1):
                    for k in range(outer_min_index[2], outer_max_index[2]+1):
                        ...
                        if not self.map[i,j,k]:
                            ...
                            rect_distance = self._aabb_distance(
                                metric_loc,
                                metric_loc + self.resolution,
                                block_min,
                                block_max)
