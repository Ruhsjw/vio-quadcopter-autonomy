...
#######################################################################
from local_replanning.code.se3_control import SE3Control
from local_replanning.code.world_traj import WorldTraj
#######################################################################

np.random.seed(0)

# Load a test world.  Pass a JSON path on the command line, or fall back to the
# first available course test file for quick local debugging.
parser = argparse.ArgumentParser()
parser.add_argument("world", nargs="?")
args, _ = parser.parse_known_args()
if args.world:
    file = Path(args.world)
else:
    util_dir = Path(inspect.getsourcefile(lambda:0)).parent.resolve() / '..' / 'util'
    candidates = sorted(util_dir.glob('test_*.json'))
    if not candidates:
        raise FileNotFoundError("No test_*.json files found for sandbox debugging.")
    file = candidates[0]
